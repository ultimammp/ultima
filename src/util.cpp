...
const char * const BITCOIN_CONF_FILENAME = "ultima.conf";
const char * const BITCOIN_PID_FILENAME = "ultimad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ultima" is a composite category enabling all Ultima-related debug output
            if(ptrCategory->count(string("ultima"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UltimaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UltimaCore
    // Mac: ~/Library/Application Support/UltimaCore
    // Unix: ~/.ultimacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UltimaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UltimaCore";
#else
    // Unix
    return pathRet / ".ultimacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ultima.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ultima.conf
