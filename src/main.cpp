...
#if defined(NDEBUG)
# error "Ultima Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        return 315360 * COIN; // project treasury
    }
	
	if (nPrevHeight < 20160) {
		nSubsidy = 0.2 * COIN;	// period with low base rewards
	}else if (nPrevHeight < 60480){
		nSubsidy = 2 * COIN;  // Standard reward I
	}else if (nPrevHeight < 100800){
		nSubsidy = 2.5 * COIN;  // Standard reward II
	}else{
		nSubsidy = 3 * COIN;  // Standard reward III
	}
	

    // base reward yearly decline by 10%, estimated 31.4m ULT in 2050
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // Reward share increase:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 20; //  +5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 20; //  +5%
	if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 20; //  +5%
	if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 20; //  +5%
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // ULTIMA : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ULT): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ULT): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ULTIMA : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ULT): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ULT): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Ultima Related Inventory Messages
