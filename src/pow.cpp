...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    /* current difficulty formula, ultima - DarkGravity v3, written by Evan Duffield - evan@ultima.org */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // mainet/regtest have the same DGW activation height - 100
    ...
    // testnet -- same start as for the above 
    } else {
        if (pindexLast->nHeight + 1 >= 100) retarget = DIFF_DGW;
        else retarget = DIFF_BTC;
    }
